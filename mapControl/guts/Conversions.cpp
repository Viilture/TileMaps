...
@brief Преобразует координаты LLA (широта, долгота, высота) в координаты XYZ

@param wlat Широта в градусах

@param wlon Долгота в градусах

@param walt Высота в метрах

@return QVector3D, представляющий координаты XYZ
Функция принимает на вход координаты LLA (широта, долгота, высота) и возвращает соответствующие координаты XYZ.
Для расчета используются формулы, основанные на параметрах эллипсоида Земли и эксцентриситете нормального эллипсоида.
Функция также проверяет корректность входных данных и выводит сообщение об ошибке, если широта или долгота находятся за пределами допустимых значений.
*/
QVector3D Conversions::lla2xyz(qreal wlat, qreal wlon, qreal walt)
{
    QVector3D toRet;

    // Проверяем, что широта и долгота находятся в допустимых пределах
    ...
    // Рассчитываем радиус кривизны меридионального сечения земли на данной широте
    qreal r_n = A_EARTH/sqrt(1.0 - NAV_E2*slat*slat);

    // Рассчитываем координаты X, Y и Z в системе координат XYZ
    toRet.setX((r_n + walt)*clat*cos(wlon*deg2rad));
    toRet.setY((r_n + walt)*clat*sin(wlon*deg2rad));
    toRet.setZ((r_n*(1.0 - NAV_E2) + walt)*slat);

    // Возвращаем вектор с найденными координатами
    return toRet;
}
...
void Conversions::test()
{
    ...
    qDebug() << degreesLatPerMeter(25.0);
    qDebug() << degreesLonPerMeter(25.0);
}
