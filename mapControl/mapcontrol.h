...
public:
    ...
    std::shared_ptr<mapMeasure> createMeasure();
    /** @brief Удалить Линейку */
    void removeMeasure(std::shared_ptr<mapMeasure> mapMeasure);

    /** @brief Создаем новый обьект отображения целей на карте */
    std::shared_ptr<mapTarget> createTarget();

    /** @brief Удаляем обьект отображения целей на карте */
    void removeTarget(std::shared_ptr<mapTarget> mapTarg);

    /** @brief Создаем новый обьект отображения целей на карте */
    std::shared_ptr<TargetControl> createTargetControl();

    /** @brief Удаляем обьект отображения целей на карте */
    void removeTargetControl(std::shared_ptr<TargetControl> mapTarg);
...
private:
    ...
    std::set<std::shared_ptr<mapMeasure>> mapMeasures;
    std::set<std::shared_ptr<mapTarget>> mapTargets;
    std::set<std::shared_ptr<TargetControl>> mapTargContrs;
