...
void MapControl::init()
{
    ...
    set = QSharedPointer<QSettings>::create(QCoreApplication::applicationDirPath() + Barier2INI, QSettings::IniFormat);
...
void MapControl::zoomUpdates()
{
    ...
    qDebug() << QTime::currentTime().msecsSinceStartOfDay() - time.msecsSinceStartOfDay();
}
...
void MapControl::doTileLayout()
{
    ...
    if(updateAll == false)
    {
        ...
        if((QTime::currentTime().msecsSinceStartOfDay() - oldTimer.msecsSinceStartOfDay()) < 200) return;
    }
...
std::shared_ptr<mapMeasure> MapControl::createMeasure()
{

    std::shared_ptr<mapMeasure> mapMeas;
    ...
    mapMeas = std::make_shared<mapMeasure>(this);
...
void MapControl::removeMeasure(std::shared_ptr<mapMeasure> mapMeasure)
{
    if(mapMeasure == nullptr) return;

    mapMeasures.erase(mapMeasure);

    scenControl->scene()->removeItem(mapMeasure.get());
}

/** @brief Создаем новый обьект отображения целей на карте */
std::shared_ptr<mapTarget> MapControl::createTarget()
{
    std::shared_ptr<mapTarget> mapTarget;
    ...
    mapTarget = std::make_shared<class mapTarget>(this);
...
void MapControl::removeTarget(std::shared_ptr<mapTarget> mapTarg)
{
    if(mapTarg == nullptr) return;

    mapTargets.erase(mapTarg);

    scenControl->scene()->removeItem(mapTarg.get());
}

std::shared_ptr<TargetControl> MapControl::createTargetControl()
{
    std::shared_ptr<TargetControl> targControl;
    ...
    targControl = std::make_shared<TargetControl>(this);
...
void MapControl::removeTargetControl(std::shared_ptr<TargetControl> trgControl)
{
    if(trgControl == nullptr) return;

    trgControl->removeAllTargets();
    mapTargContrs.erase(trgControl);
}
