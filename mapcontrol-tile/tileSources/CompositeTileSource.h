...
class CompositeTileSource : public MapTileSource
{
    Q_OBJECT
public:
    explicit CompositeTileSource(QObject *parent = nullptr);
    ...
    const QList<QSharedPointer<MapTileSource> > &childSources() const;
...
/*! \brief выдается, когда что-либо меняется в слоях. Один из них добавляется / удаляется, перемещается, изменяется прозрачность и т.д.
*/
    void sourcesChanged();
...
//Хэш QString:Указатель QMap на quint32:Указатель QImage
